...
export default function CreateQuizScreen() {
  ...
  const [createQuizMutation, { loading }] = useMutation<{ createQuiz: number }>(CREATE_QUIZ);

  const handleAddQuestion = () => {
    setQuestions((prev) => [
      ...prev,
      { id: nextId(), text: '', options: ['', '', '', ''], correctAnswers: [], type: 'one', explanation: '' },
    ]);
  };
  ...
  const handleAddTag = () => {
    ...
    if (!name) return;
    setTags((prev) => [...prev, { id: nextId(), name, editing: false }]);
    setNewTagText('');
  };
  ...
  const handleCreateQuiz = async () => {
    try {
      ...
      navigation.navigate('QuizDetails', { id: result.data?.createQuiz });
    } catch (err: any) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <Text style={styles.screenTitle}>Create quiz</Text>
        ...
        <Button onPress={handleCreateQuiz} loading={loading} style={styles.submitBtn}>
          Create quiz
        </Button>
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalDesc}>
              Describe what you want the question to be about.
            </Text>
            <TextInput
...
const styles = StyleSheet.create({
  ...
  modalOverlay: {
    flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-start',
  },
  modalBox: {
